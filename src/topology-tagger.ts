...
import { Graph as AnalysisGraph, alg as GraphLibAlg, Edge, Graph} from "graphlib";
...
export function getAnalysisGraph(sageModelGraph: ISageGraph, swapTransferSource = false) {
  ...
  const setEdge = (s, t, link: ISageLink, label:string) => {
    const linkData = {
      title: link.title,
...
function edgesLength(edges: Array<Edge>|void):number {
  return edges ? edges.length : 0;
}

function countMultiLinkTargetNodes(g:AnalysisGraph) {
  ...
  return g.nodes().filter((node: string) => edgesLength(g.inEdges(node)) ? (g.inEdges(node) as Array<Edge>).length : 0 > 1).length;
}
...
function countLinearGraphs(g:Graph) {
  ...
  function isLinear(subGraph: string[]) {
    const atMost1InOutEdges = (node: string) => {
      return edgesLength(g.inEdges(node)) <= 1 && edgesLength(g.outEdges(node)) <= 1;
    };
    const nodesWithAtMost1inAnd1outEdge =
      subGraph.filter((node: string) => (atMost1InOutEdges(node))).length;
    const numberOfInEdges =
      subGraph.map((node: string) => edgesLength(g.inEdges(node)))
...
function countGraphsWithFeedback(g) {
  ...
  const hasFeedback = (subGraph:any) => _.intersection(subGraph, cycles).length > 0 ;
...
export function getTopology(sageModelGraph: ISageGraph):ITopoReport {
